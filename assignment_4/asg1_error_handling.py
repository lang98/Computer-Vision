...
class Figure:
    ...
    def try_unsafe_handler(self, unsafe_handler, e):
        try:
            unsafe_handler(e)
        except KeyboardInterrupt:
            raise
        except:
            self.fig.text(0.5, 0.5, traceback.format_exc(), color='red', backgroundcolor='black', ha='center', fontsize=10)
            self.draw()

    def raise_mouse_down(self, e):
        if not self.mouse_down_handler is None:
            self.try_unsafe_handler(self.mouse_down_handler, e)

    def raise_mouse_up(self, e):
        if not self.mouse_up_handler is None:
            self.try_unsafe_handler(self.mouse_up_handler, e)

    def raise_mouse_over(self, e):
        if not self.mouse_over_handler is None:
            self.try_unsafe_handler(self.mouse_over_handler, e)

    def raise_mouse_wheel(self, e):
        if not self.mouse_wheel_handler is None:
            self.try_unsafe_handler(self.mouse_wheel_handler, e)

    def raise_key_down(self, e):
        if not self.key_down_handler is None:
            self.try_unsafe_handler(self.key_down_handler, e)

    def raise_key_up(self, e):
        if not self.key_up_handler is None:
            self.try_unsafe_handler(self.key_up_handler, e)
...
class KmeansPresenter:
    ...
    def on_key_down(self, e): # do the following when certain "key" is pressed
        if e.key == 'i':    # make 1 iteration of K-means
            ...
            self.fig.draw()
        if e.key == 'c':    # run K-means to convergence (when SSE (energy) improvement is less than
                            # given threshold)
            delta = np.infty
            while (delta > 0.001):
